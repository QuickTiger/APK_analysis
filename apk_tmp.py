...
api_information=[["SmsReceiver;->abortBroadcast","拦截短信接收","10"],["/root/su","切换用户","10"],["/system/bin/sh","执行shell","9"],["java/lang/Runtime;->exec","执行字符串命令","7"],["mount -o remount","重新挂载档案系统","7"],["ActivityManager;->killBackgroundProcesses","中断进程，可用于关闭杀软","6"],["ActivityManagerNative;->killBackgroundProcesses","中断进程，可用于关闭杀软","6"],["ActivityManagerNative;->restartPackage"," 中断进程，可用于关闭杀软","6"],["ActivityManager;->restartPackage"," 中断进程，可用于关闭杀软","6"],["ContentResolver;->query","读取联系人、短信等数据库","6"],["Telephony$Mms;->query","读取短信","6"],["ContentResolver;->delete","删除短信、联系人","6"],["/proc/mounts","加载文件系统","6"],["ActivityManager$Stub$Proxy;->shutdown","关机","5"],["PackageManager;->installPackage","安装apk包","5"],["SmsManager;->getAllMessagesFromSim","获取sim卡上的短信","5"],["SmsManager;->sendTextMessage","发送普通短信","5"],["internal/telephony/ISms$Stub$Proxy;->sendData","发送短信","5"],["internal/telephony/ISms$Stub$Proxy;->sendMultipartText","发送短信","5"],["internal/telephony/ISms$Stub$Proxy;->sendText","发送短信","5"],["internal/telephony/ITelephony$Stub$Proxy;->call","拔打电话","5"],["getRuntime","获取命令行环境","5"],["content://sms","获取短信数据库","5"],["MediaRecorder;->setAudioSource","开启录音功能","4"],["MediaRecorder;->setVideoSource","开启视频录制","4"],["Camera;->open","开启相机","3"],["PowerManager;->reboot","重启手机","3"],["TelephonyManager;->getLine1Number","获取手机号","3"],["SmsManager;->sendMultipartTextMessage","发送彩信","3"],["chmod ","更改文件权限","3"],["BluetoothSocket;->connect","连接蓝牙","2"],["IBluetoothPbap$Stub$Proxy;->connect","连接蓝牙","2"],["LocationManager;->getLastKnownLocation","获取地址位置","2"],["TelephonyManager;->getDeviceId","搜集用户手机IMEI码、电话号码、系统版本号等信息","2"],["TelephonyManager;->getSimSerialNumber()","获取SIM序列号","2"],["SmsManager;->sendDataMessage","发送二进制消息","2"],["content://telephony/carriers","获取所有的APN（网络接入点）配置信息","2"],["BluetoothAdapter;->enable","开启蓝牙","1"],["ContentService;->dump","转储联系人、短信等信息","1"],["Downloads$ByUri;->startDownloadByUri","下载文件","1"],["Downloads$DownloadBase;->startDownloadByUri","下载文件","1"],["Settings$Bookmarks;->add","添加浏览器书签","1"],["SpeechRecognizer;->startListening","开启麦克风","1"],["WifiManager;->setWifiEnabled","开启WIFI","1"],["http/multipart/FilePart;->sendData","发送http请求","1"],["http/multipart/Part;->send","发送http请求","1"],["http/multipart/Part;->sendParts","发送http请求","1"],["http/multipart/StringPart;->sendData","发送http请求","1"],["java/net/HttpURLConnection;->connect","连接URL","1"],["java/net/URL;->getContent","获取网页内容","1"],["java/net/URL;->openConnection","连接URL","1"],["java/net/URLConnection;->connect","连接URL","1"],["DefaultHttpClient;->execute","发送HTTP请求","1"],["HttpClient;->execute","请求远程服务器","1"],["content://telephony/carriers/preferapn","可能用于篡改APN（网络接入点）以调用应用市场M-Market扣费接口并验证","1"],["content://browser/bookmarks","获取浏览器书签","1"],["/system/bin/cp","复制文件","1"]]
...
apis_file=open("config/default_api_information")
apis_result=open("/tmp/"+apk_name+"/apis.txt","w")
apis_result.close()
for line in apis_file:
	if not line.startswith("#") and line.find("L")>-1:
		os.system("egrep -n -r \""+line.split("\n")[0]+"\" /tmp/"+apk_name+"/smali |wc -l >>/tmp/"+apk_name+"/apis.txt")
		os.system("echo  -n \""+line+"\" >>/tmp/"+apk_name+"/apis.txt")
tmpf=open("/tmp/"+apk_name+"/apis.txt")
tmpf1=open("/tmp/"+apk_name+"/apis_result_tmp.txt","w")
data ,num = re.subn("(?<=[\d])\n","\t",tmpf.read())
tmpf1.write(data)
tmpf.close()
tmpf1.close()
apis_result=open("/tmp/"+apk_name+"/apis_result_tmp.txt")


for line in apis_result:
	num=line.split("\t")[0]
	api=line.split("\t")[1]
	for apis_info in api_information:
		if apis_info[0] == api.split("\n")[0] and not num == "0":
